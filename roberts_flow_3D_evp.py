...
This script solves the full 3D eigenvalue problem to find peak growth rates.  It is quite slow compared to the 2.5D scripts in this repo, and should likely be used for comparison purposes only.

Usage:
    roberts_flow_3D_evp.py [options]
...
ax.scatter(np.abs(kxs[mask]), eigs.imag[mask], label=r'$\omega_I$')
ax.axhline(y=0, alpha=0.5, color='xkcd:grey')
ax.set_xscale('log')
ax.set_xlabel('|kx|')
ax.set_ylabel('omega')
ax.legend()
fig.savefig(data_dir+'/evals_omega_abskx.png', dpi=300)
ax.scatter(np.abs(kxs[~mask]), eigs.real[~mask], alpha=0.2)
ax.scatter(np.abs(kxs[~mask]), eigs.imag[~mask], alpha=0.2)
fig.savefig(data_dir+'/evals_omega_abskx_all.png', dpi=300)
